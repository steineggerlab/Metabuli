...
void Classifier::analyseResult(const char * queryFileName) {
    char suffixedResultFileName[1000];
    sprintf(suffixedResultFileName,"%s_result", queryFileName);
    struct MmapedData<MatchedKmer> resultFile = mmapData<MatchedKmer>(suffixedResultFileName);
    SORT_PARALLEL(resultFile.data, resultFile.data + resultFile.fileSize / sizeof(MatchedKmer), [=](MatchedKmer x, MatchedKmer y) { return x.queryID < y.queryID; });
