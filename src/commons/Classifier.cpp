    ...
    if(threadNum == 1){
        ...
        for(size_t tSplitCnt = 0; tSplitCnt < numOfDiffIdxSplits_use; tSplitCnt++){
            ...
            if(queryAA < AminoAcid(diffIdxSplits.data[tSplitCnt].ADkmer)){
                ...
                splits.emplace_back(splitWidth, queryKmerCnt - 1, queryKmerCnt - splitWidth, diffIdxSplits.data[tSplitCnt]);
                break;
            }
        }
    } else{
...
void Classifier::getBestGenusLevelMatchCombination(vector<ConsecutiveMatches> & chosenMatchCombination, Match * matchList, size_t end, size_t offset){
    ...
    TaxID currentTaxID;
    if(matchList[offset].queryId == 2120) {
        for (size_t i2 = offset; i2 < end + 1; i2++) {
            cout << matchList[i2].queryId << " " << matchList[i2].taxID << " "<<matchList[i2].position<<" "<<(int)matchList[i2].frame<<"\n";
        }
        cout<<"after"<<endl;
        cout << matchList[end + 1].queryId << " " << matchList[end + 1].taxID << " "<<matchList[end + 1].position<<" "<<(int)matchList[end + 1].frame<<"\n";
    }
