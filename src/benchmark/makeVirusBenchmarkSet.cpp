...
int makeVirusBenchmarkSet(int argc, const char **argv, const Command &command) {
    std::srand(4);
    LocalParameters &par = LocalParameters::getLocalInstance();
    par.parseParameters(argc, argv, command, false, Parameters::PARSE_ALLOW_EMPTY, 0);
    string assemblyList = par.filenames[0];
    string taxonomyPath = par.filenames[1];
    string accession2taxidFilePath = par.filenames[2];
    ...
    FILE *handle = fopen(accession2taxidFilePath.c_str(), "r");
    if (handle == NULL) {
        Debug(Debug::ERROR) << "Could not open " << accession2taxidFilePath << " for reading\n";
        EXIT(EXIT_FAILURE);
    }
    char buffer[1024];
    int taxID;
    ...
    while (getline(assemblyListFile, assemblyAccession)) {
        ...
        if (assacc2taxid.find(assemblyAccession) == assacc2taxid.end()) {
            cerr << "Error: accession " << assemblyAccession << " not found in the assembly accession to taxid mapping" << endl;
            return 1;
        } else {
    ...
    // randomly choose 1/4 families with multiple genera without replacement
    int n = int(familyWithMultipleGenera.size() / 4);
